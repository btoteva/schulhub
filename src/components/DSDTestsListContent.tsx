...
const MODELLSATZ_5_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  "13": FaClipboardList,
  "14": FaHeadphones,
};
const MODELLSATZ_6_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  "15": FaClipboardList,
  "16": FaHeadphones,
};
const MODELLSATZ_7_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  "17": FaClipboardList,
  "18": FaHeadphones,
};
      ...
      <ul className="space-y-4">
        ...
        <li>
          ...
          >
            <button
              type="button"
              onClick={() => setModellsatz5Open((open) => !open)}
              className={`flex items-center gap-4 w-full p-6 text-left transition-colors ${isLight ? "hover:bg-slate-50" : "hover:bg-gray-800/70"}`}
            >
              <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-amber-600/80 flex items-center justify-center overflow-hidden">
                <img src="https://i.imgur.com/tf1QyRC.png" alt="" className="w-full h-full object-contain p-1" />
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`text-xl font-bold ${isLight ? "text-slate-800" : "text-white"}`}>
                  {language === "bg" ? "DSD I Моделен тест 5" : "DSD I Modellsatz 5"}
                </h3>
                <p className={isLight ? "text-slate-800 text-sm mt-1" : "text-gray-400 text-sm mt-1"}>
                  {language === "bg"
                    ? "Части от един изпит: Leseverstehen, Hörverstehen, Schriftliche Kommunikation"
              ...
              <span className={isLight ? "flex-shrink-0 text-amber-600" : "flex-shrink-0 text-amber-400"}>
                {modellsatz5Open ? <FaChevronUp className="w-5 h-5" /> : <FaChevronDown className="w-5 h-5" />}
              </span>
            </button>
            {modellsatz5Open && (
              <div className={`px-4 pb-4 pt-0 space-y-2 border-t ${isLight ? "bg-white border-amber-500/30" : "bg-gray-800/50 border-amber-500/20"}`}>
                {modellsatz5Tests.map((test) => (
                  <div key={test.id}>{renderTestLink(test, MODELLSATZ_5_ICONS[test.id])}</div>
                ))}
              </div>
            )}
          </div>
        ...
        <li>
          ...
          >
            <button
              type="button"
              onClick={() => setModellsatz6Open((open) => !open)}
              className={`flex items-center gap-4 w-full p-6 text-left transition-colors ${isLight ? "hover:bg-slate-50" : "hover:bg-gray-800/70"}`}
            >
              <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-amber-600/80 flex items-center justify-center overflow-hidden">
                <img src="https://i.imgur.com/tf1QyRC.png" alt="" className="w-full h-full object-contain p-1" />
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`text-xl font-bold ${isLight ? "text-slate-800" : "text-white"}`}>
                  {language === "bg" ? "DSD I Моделен тест 6" : "DSD I Modellsatz 6"}
                </h3>
                <p className={isLight ? "text-slate-800 text-sm mt-1" : "text-gray-400 text-sm mt-1"}>
                  {language === "bg"
                    ? "Части от един изпит: Leseverstehen, Hörverstehen, Schriftliche Kommunikation"
              ...
              <span className={isLight ? "flex-shrink-0 text-amber-600" : "flex-shrink-0 text-amber-400"}>
                {modellsatz6Open ? <FaChevronUp className="w-5 h-5" /> : <FaChevronDown className="w-5 h-5" />}
              </span>
            </button>
            {modellsatz6Open && (
              <div className={`px-4 pb-4 pt-0 space-y-2 border-t ${isLight ? "bg-white border-amber-500/30" : "bg-gray-800/50 border-amber-500/20"}`}>
                {modellsatz6Tests.map((test) => (
                  <div key={test.id}>{renderTestLink(test, MODELLSATZ_6_ICONS[test.id])}</div>
                ))}
              </div>
            )}
          </div>
        ...
        <li>
          ...
          >
            <button
              type="button"
              onClick={() => setModellsatz7Open((open) => !open)}
              className={`flex items-center gap-4 w-full p-6 text-left transition-colors ${isLight ? "hover:bg-slate-50" : "hover:bg-gray-800/70"}`}
            >
              <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-amber-600/80 flex items-center justify-center overflow-hidden">
                <img src="https://i.imgur.com/tf1QyRC.png" alt="" className="w-full h-full object-contain p-1" />
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`text-xl font-bold ${isLight ? "text-slate-800" : "text-white"}`}>
                  {language === "bg" ? "DSD I Моделен тест 7" : "DSD I Modellsatz 7"}
                </h3>
                <p className={isLight ? "text-slate-800 text-sm mt-1" : "text-gray-400 text-sm mt-1"}>
                  {language === "bg"
                    ? "Части от един изпит: Leseverstehen, Hörverstehen, Schriftliche Kommunikation"
              ...
              <span className={isLight ? "flex-shrink-0 text-amber-600" : "flex-shrink-0 text-amber-400"}>
                {modellsatz7Open ? <FaChevronUp className="w-5 h-5" /> : <FaChevronDown className="w-5 h-5" />}
              </span>
            </button>
            {modellsatz7Open && (
              <div className={`px-4 pb-4 pt-0 space-y-2 border-t ${isLight ? "bg-white border-amber-500/30" : "bg-gray-800/50 border-amber-500/20"}`}>
