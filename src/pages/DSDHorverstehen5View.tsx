...
import horverstehenData from "../data/dsd-horverstehen-5.json";

const STORAGE_KEY = "schulhub-dsd-horverstehen-5";
...
const DSDHorverstehen5View: React.FC = () => {
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        {teile.length > 0 ? (
                      ...
                      <table className={`w-full border-collapse rounded-xl overflow-hidden ${isLight ? "border border-slate-300" : "border border-gray-600"}`}>
                        ...
                        <tbody>
                          {teil.teil3Exercise.tasks.map((task) => (
                            <tr key={task.id} className={isLight ? "bg-slate-100" : "bg-gray-900/30"}>
          ...
          <section className={`mt-12 p-6 rounded-xl border-2 ${isLight ? "border-amber-500/50 bg-amber-50/50" : "border-amber-500/50 bg-amber-900/10"}`}>
            <p className={`text-center mb-4 ${isLight ? "text-slate-700" : "text-gray-300"}`}>
              Du hast jetzt 10 Minuten Zeit, um deine Lösungen auf das Antwortblatt zu übertragen.
            </p>
            <p className={`font-semibold text-center ${isLight ? "text-amber-600" : "text-amber-400"}`}>
...
export default DSDHorverstehen5View;
