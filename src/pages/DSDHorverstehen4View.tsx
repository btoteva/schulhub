...
const STORAGE_KEY = "schulhub-dsd-horverstehen-4";
          ...
          <>
            ...
            <div className="space-y-10">
              {data.teile.map((teil) => (
                <section
                  key={teil.id}
                  className="bg-white dark:bg-gray-800/50 rounded-xl p-8 border border-amber-500/30"
                >
                  <h2 className="text-2xl font-bold text-amber-600 dark:text-amber-400 mb-2">{teil.title}</h2>
                  <h3 className="text-xl font-semibold text-amber-700 dark:text-amber-200 mb-4">{teil.subtitle}</h3>

                  {(teil as { intro?: string }).intro && (
                    <p className="text-slate-900 dark:text-gray-300 mb-6 leading-relaxed">{(teil as { intro: string }).intro}</p>
                  )}
                  ...
                  {(teil as { audioUrl?: string }).audioUrl && (
                    <div className="mb-8 p-6 bg-slate-100 dark:bg-gray-900/50 rounded-xl border border-amber-500/30">
                      <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                        <p className="text-amber-700 dark:text-amber-200 font-semibold flex items-center gap-2">
                      ...
                      <ControlledAudio className="w-full max-w-2xl" src={(teil as { audioUrl: string }).audioUrl}>
                        Вашият браузър не поддържа възпроизвеждане на аудио.
                      </ControlledAudio>
                    </div>
                  )}

                  {teil.scenes && teil.scenes.length > 0 && (showTeacherTextTeile[teil.id] || !(teil as { audioUrl?: string }).audioUrl) && (
                    <div className="space-y-8">
                      {teil.scenes.map((scene) => (
                        <div key={scene.id} className="border border-slate-300 dark:border-gray-600 rounded-xl p-6 bg-slate-100 dark:bg-gray-900/30">
                          <h4 className="text-lg font-bold text-amber-700 dark:text-amber-200 mb-4">Szene {scene.id}</h4>
                          <div className="space-y-3">
                            {scene.lines.map((line, i) => (
                              <p key={i} className="text-slate-900 dark:text-gray-200 leading-relaxed">
                                <span className="font-semibold text-amber-700 dark:text-amber-200">{line.speaker}: </span>
                                {line.text}
                              </p>
                            ))}
                          </div>
                  ...
                  {teil.aufgaben && teil.aufgaben.length > 0 && (showTeacherTextTeile[teil.id] || !(teil as { audioUrl?: string }).audioUrl) && (
                    <div className="space-y-8">
                      {teil.aufgaben.map((aufgabe) => (
                        <div key={aufgabe.id} className="border border-slate-300 dark:border-gray-600 rounded-xl p-6 bg-slate-100 dark:bg-gray-900/30">
                          <h4 className="text-lg font-bold text-amber-700 dark:text-amber-200 mb-4">{(teil as { itemLabel?: string }).itemLabel || "Aufgabe"} {aufgabe.id}</h4>
                  ...
                  {teil.content && (showTeacherTextTeile[teil.id] || !(teil as { audioUrl?: string }).audioUrl) && (
                    <div className="border border-slate-300 dark:border-gray-600 rounded-xl p-6 bg-slate-100 dark:bg-gray-900/30">
                      <p className="text-slate-900 dark:text-gray-200 leading-relaxed whitespace-pre-line">{teil.content}</p>
                    </div>
                  )}
                    ...
                    <div className="mt-10 pt-10 border-t-2 border-amber-500/40">
                      ...
                      <div className="mt-8 pt-6 border-t border-amber-500/30">
                        ...
                        {showTeil1Answers && teil.teil1Exercise.correctAnswers && (
                          <div className="mt-6">
                            <p className="text-amber-800 dark:text-amber-200 font-bold mb-3">Teil 1: Sport – Lösungen</p>
                            <ul className="list-disc list-inside text-slate-900 dark:text-gray-200 space-y-1">
                              {[1, 2, 3, 4, 5].map((nr) => (
                  ...
                  {teil.id === "teil2" && teil.teil2Exercise && (
                    <div className="mt-10 pt-10 border-t-2 border-amber-500/40">
                      ...
                      <div className="mt-8 pt-6 border-t border-amber-500/30">
                        ...
                        {showTeil2Answers && (
                          <div className="mt-6">
                            <p className="text-amber-800 dark:text-amber-200 font-bold mb-3">Teil 2: Durchsagen im Zug – Lösungen</p>
                  ...
                  {teil.id === "teil3" && teil.teil3Exercise && (
                    <div className="mt-10 pt-10 border-t-2 border-amber-500/40">
                      ...
                      <div className="mt-8 pt-6 border-t border-amber-500/30">
                        ...
                        {showTeil3Answers && (
                          <div className="mt-6">
                            <p className="text-amber-800 dark:text-amber-200 font-bold mb-3">Teil 3: Interview mit Jan Thiel – Lösungen</p>
                            <ul className="list-disc list-inside text-slate-900 dark:text-gray-200 space-y-2">
                  ...
                  {teil.id === "teil4" && teil.teil4Exercise && (
                    <div className="mt-10 pt-10 border-t-2 border-amber-500/40">
                      ...
                      <div className="mt-8 pt-6 border-t border-amber-500/30">
                        ...
                        {showTeil4Answers && (
                          <div className="mt-6">
                            <p className="text-amber-800 dark:text-amber-200 font-bold mb-3">Teil 4: Reise nach Australien – Lösungen</p>
                            <ul className="list-disc list-inside text-slate-900 dark:text-gray-200 space-y-2">
                  ...
                  {teil.id === "teil5" && teil.teil5Exercise && (
                    <div className="mt-10 pt-10 border-t-2 border-amber-500/40">
                      ...
                      <p className={`text-slate-900 dark:text-gray-300 ${teil.teil5Exercise.instruction6 ? "mb-2" : "mb-8"}`}>{teil.teil5Exercise.instruction5}</p>
                      {teil.teil5Exercise.instruction6 && <p className="text-slate-900 dark:text-gray-300 mb-8">{teil.teil5Exercise.instruction6}</p>}
                      <div className="flex flex-col md:flex-row gap-8 mb-8">
                        <div className="flex-shrink-0">
                          <p className="text-amber-800 dark:text-amber-200 font-semibold mb-3">Aussagen A–H</p>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <table className="w-full border-collapse border border-slate-300 dark:border-gray-600 rounded-xl overflow-hidden">
                            <thead>
                              <tr className="bg-slate-200 dark:bg-gray-700/70">
                                <th className="border border-slate-300 dark:border-gray-600 px-4 py-2 text-amber-800 dark:text-amber-200 font-semibold text-left w-20">Aufgabe</th>
                      ...
                      <div className="mt-8 pt-6 border-t border-amber-500/30">
                        ...
                        {showTeil5Answers && (
                          <div className="mt-6">
                            <p className="text-amber-800 dark:text-amber-200 font-bold mb-3">Teil 5: Sonne und Schönheit – Lösungen</p>
                            <ul className="list-disc list-inside text-slate-900 dark:text-gray-200 space-y-2">
                              {teil.teil5Exercise.taskNumbers.map((nr) => (
...
export default DSDHorverstehen4View;
