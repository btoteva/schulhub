...
import dsdData from "../data/dsd-modellsatz-6.json";

const STORAGE_KEY = "schulhub-dsd-modellsatz-5-v4";
...
const DSDModellsatz5View: React.FC = () => {
              ...
              <p className="text-slate-900 dark:text-gray-300 leading-relaxed mb-6">
                {(() => {
                  ...
                  for (const task of sortedTasks) {
                    ...
                    const ph1 = `(${t.gap}) ________`;
                    const ph2Match = rest.match(new RegExp(`_{6,}\\s*\\(${t.gap}\\)`));
                    let idx = rest.indexOf(ph1);
                    let phLen = ph1.length;
                    if (idx === -1 && ph2Match) {
                      idx = rest.indexOf(ph2Match[0]);
                      phLen = ph2Match[0].length;
                    }
                    if (idx === -1) continue;
                    segments.push(rest.slice(0, idx));
                    segments.push({ type: "gap", task: t });
                    rest = rest.slice(idx + phLen);
                  }
                  segments.push(rest);
          ...
          <section className="mt-12 p-6 rounded-xl border-2 border-amber-500/50 bg-amber-900/10">
            ...
            {showAnswerKey && data.teile.length > 0 && (
              <div className="mt-8 p-6 bg-slate-200 dark:bg-gray-900/80 rounded-xl border border-amber-500/30 text-left">
                <h3 className="text-xl font-bold text-amber-600 dark:text-amber-400 mb-6">Deutsches Sprachdiplom der KMK DSD I – Leseverstehen Lösungsschlüssel (Modellsatz 5)</h3>
                <div className="space-y-5 text-slate-900 dark:text-gray-300 text-sm leading-relaxed">
                  {teil1 && (
                    <div>
                      ...
                      <p>
                        {teil1.tasks?.slice().sort((a, b) => (a as { id: number }).id - (b as { id: number }).id).map((t) => (
                          <span key={t.id}>{(t as { id: number }).id} {(t as { correct: string }).correct} &nbsp;&nbsp; </span>
                        ))}
                        {teil1.aufgabe5 && (() => {
                          const opt = teil1.aufgabe5.options.find((o) => o.correct);
                          return opt ? <span key="5">5 {opt.id}</span> : null;
                        })()}
                      </p>
                    </div>
                  )}
                  {teil2 && teil2.summaries && (
                    <div>
                      ...
                      <p>
                        {teil2.summaries.filter((s) => !s.isExample).sort((a, b) => a.id - b.id).map((s) => (
                          <span key={s.id}>{s.id} {s.correct} &nbsp;&nbsp; </span>
                        ))}
                      </p>
                    </div>
                  )}
                  {teil3 && teil3.tasks && (
                    <div>
                      ...
                      <p>
                        {teil3.tasks.slice().sort((a, b) => (a as { id: number }).id - (b as { id: number }).id).map((t) => (
                          <span key={(t as { id: number }).id}>{(t as { id: number }).id} {(t as { correct: string }).correct} &nbsp;&nbsp; </span>
                        ))}
                      </p>
                    </div>
                  )}
                  {teil4 && teil4.questions && (
                    <div>
                      ...
                      <p>
                        {teil4.questions.slice().sort((a, b) => a.id - b.id).map((q) => {
                          const opt = q.options.find((o) => o.correct);
                          return opt ? <span key={q.id}>{q.id} {opt.id} &nbsp;&nbsp; </span> : null;
                        })}
                      </p>
                    </div>
                  )}
                  {teil5 && teil5.summaries && (
                    <div>
                      ...
                      <p>
                        {teil5.summaries.filter((s) => !(s as { isExample?: boolean }).isExample).sort((a, b) => a.id - b.id).map((s) => (
                          <span key={s.id}>{s.id} {s.correct} &nbsp;&nbsp; </span>
                        ))}
                      </p>
                    </div>
...
export default DSDModellsatz5View;
