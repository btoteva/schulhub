...
const GermanFotoPodcastView: React.FC = () => {
  ...
  const fromState =
    (location.state as
      | { fromAllList?: boolean; episode?: GermanPodcastItem }
      | undefined) || undefined;

  const decodedId = episodeId ? decodeURIComponent(episodeId) : undefined;
  ...
  const episode: GermanPodcastItem =
    fromState?.episode ??
    fotoPodcasts.find((p) => p.id === decodedId) ??
    (decodedId ? feedItems.find((p) => p.id === decodedId) : undefined) ??
    fotoPodcasts[0];
    ...
    >
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="rounded-xl overflow-hidden shadow-xl mb-8">
          <div className="bg-gradient-to-br from-amber-700 to-orange-900 py-8 px-6 flex flex-col sm:flex-row items-center gap-6 min-h-[140px]">
            {episode.imageUrl || channel?.imageUrl ? (
              ...
              <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-xl bg-amber-500/30 flex items-center justify-center flex-shrink-0">
                <FaCamera className="w-16 h-16 text-white" />
              </div>
            )}
        ...
        {episode.audioUrl ? (
          <div
            className={`rounded-xl overflow-hidden shadow-xl p-4 sm:p-6 mb-6 ${isLight ? "bg-slate-100 border border-slate-200" : "bg-slate-800/50 border border-slate-600"}`}
          >
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
              <label
                className="text-sm font-medium shrink-0"
                htmlFor="foto-podcast-volume"
              >
                {volumeLabel}
              </label>
              <input
                id="foto-podcast-volume"
                type="range"
                min={0}
                max={1}
                step={0.05}
                ...
                className="flex-1 min-w-0 h-2 rounded-full appearance-none bg-slate-200 dark:bg-slate-600 accent-amber-600"
              />
          ...
          <p
            className={`text-sm mb-4 ${isLight ? "text-slate-500" : "text-gray-400"}`}
          >
            {language === "bg"
              ? "Аудиото за този епизод не е налично."
              : language === "de"
                ? "Das Audio für diese Episode ist nicht verfügbar."
                : "Audio for this episode is not available."}
          </p>
        )}
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          ...
          {token && (
            ...
            >
              <FaCheckCircle
                className={listened ? "w-5 h-5" : "w-5 h-5 opacity-70"}
              />
        ...
        {episode.description && (
          <div
            className={`podcast-description text-base ${isLight ? "text-slate-700" : "text-slate-300"} [&_a]:text-amber-600 [&_a]:underline [&_a:hover]:text-amber-500 [&_p]:mb-2 [&_h4]:font-semibold [&_h4]:mt-4 [&_h4]:mb-1 [&_ul]:list-disc [&_ul]:pl-6 [&_ul]:my-2 [&_li]:mb-1`}
...
export default GermanFotoPodcastView;
