...
const MessageSpace: React.FC = () => {
  ...
  const spaceId = (spaceIdParam || "").trim();
  ...
  const loadHistory = useCallback(async () => {
    ...
    if (!tok || !spaceId) return;
    try {
      const res = await fetchSpaceMessages(tok, spaceId);
      setSpaceName(res.space_name || "");
  ...
  }, [spaceId, t.messagesPermissionDenied]);

  useEffect(() => {
    if (!token || !spaceId || offline) return;
    setLoading(true);
    loadHistory();
  }, [token, spaceId, offline, loadHistory]);

  useEffect(() => {
    if (!token || !spaceId || offline || error) return;
    let cancelled = false;
    const id = window.setInterval(() => {
      if (!cancelled && !document.hidden) loadHistory();
    }, POLL_INTERVAL_MS);
    return () => {
      cancelled = true;
      window.clearInterval(id);
    };
  }, [token, spaceId, offline, error, loadHistory]);

  useEffect(() => {
    if (!listRef.current) return;
    const grew = messages.length > lastCountRef.current;
    lastCountRef.current = messages.length;
    if (grew) listRef.current.scrollTop = listRef.current.scrollHeight;
  }, [messages]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await sendSpaceMessage(token, spaceId, text);
      setMessages((prev) => {
  ...
  if (!user) return null;
  if (!spaceId) {
    return <div className="container mx-auto px-4 py-6">Invalid space</div>;
  }
    ...
    <div className="flex h-[calc(100vh-5rem)] flex-col">
      <div className="border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-800">
        <div className="container mx-auto flex max-w-3xl items-center gap-3">
          ...
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 text-white">
            <FaUsers />
          </div>
          <div className="min-w-0 flex-1">
            <div className="truncate text-base font-bold text-slate-900 dark:text-white">
              {spaceName || t.messagesSpaceType}
            </div>
                ...
                return (
                  <React.Fragment key={m.id}>
                    ...
                    <li className={`flex ${m.mine ? "justify-end" : "justify-start"}`}>
                      ...
                      >
                        {!m.mine && (
                          <p className="mb-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
                            {m.from}
                          </p>
                        )}
